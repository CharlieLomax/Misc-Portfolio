...
void SelfDestructCollision::checkCollision(GameObject* object, Mobile* other)
{
	...
	if (object->getBox().intersects(other->getBox())) {
		object->setCollision(NULL);
		object->setObj(new HiddenObject(object->getSize(), object->getPosition()));
	}
}
