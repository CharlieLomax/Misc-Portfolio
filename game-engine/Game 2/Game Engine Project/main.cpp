...
void initializeCharacter(Mobile& character, const Paddle& init) {
    character = init;
}

void makeBlocks(float width, float height, int numRows, float startY) {
    for (float i = startY; i < startY + numRows * height; i += height) {
        for (float j = 0; j < 800; j += width) {
            SelfDestructBlock* block = new SelfDestructBlock(sf::Vector2f(width, height), sf::Vector2f(j, i));
        }
    }
}
...
int main()
{
    ...
    dukglue_register_constructor<Barrier, float, float, float, float>(scripter.getContext(), "Barrier");
    dukglue_register_constructor<Paddle, float, float, float, float>(scripter.getContext(), "Paddle");
    ...
    dukglue_register_function(scripter.getContext(), &makeBlocks, "makeBlocks");
    ...
    Ball ball(sf::Vector2f(10, 10), sf::Vector2f(390, 400), sf::Vector2f(0.1f, 0.1f));
